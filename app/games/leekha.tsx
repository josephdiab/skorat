...
export default function LeekhaScreen() {
  ...
  const currentRoundPoints = (pid: string) =>
    getRoundPoints(hearts[pid] || 0, qsHolder === pid, tenHolder === pid);
  const currentTotalPoints = players.reduce(
    (sum, p) => sum + currentRoundPoints(p.id),
    0,
  );
  const isRoundValid =
    remainingHearts === 0 &&
  ...
  const toggleQS = (pid: string) =>
    setQsHolder((p) => (p === pid ? null : p !== null ? p : pid));
  const toggleTen = (pid: string) =>
    setTenHolder((p) => (p === pid ? null : p !== null ? p : pid));
  ...
  const commitRound = () => {
    ...
    if (__DEV__) {
      ...
      Logger.info(
        "SANITY_STATS",
        `📌 Leekha Round-by-Round for ${p0.name} (${p0.profileId})`,
        {
      ...
      for (const r of rounds) {
        ...
        Logger.info(
          "SANITY_STATS",
          `R${r.roundNum}: +${r.score} (total=${runningTotal})`,
          {
  ...
  const saveEditedRound = () => {
    ...
    const totalEditHearts = Object.values(editingRound.hearts).reduce(
      (a, b) => a + b,
      0,
    );
    ...
    if (
      totalEditHearts !== 13 ||
      !editingRound.qsHolder ||
      !editingRound.tenHolder
    )
      return;
  ...
  const winnersNames =
    winners.length > 0 ? winners.map((p) => p.name).join(" & ") : "No Winner";
          ...
          <View style={GameStyles.footer}>
            <TouchableOpacity
              style={[
                ...
                !isRoundValid && GameStyles.disabledButton,
              ]}
              onPress={commitRound}
            ...
            >
              <Text
                style={[
                  ...
                  !isRoundValid && { color: Colors.textMuted },
                ]}
              >
                SUBMIT ROUND
              </Text>
            ...
            <View style={GameStyles.modalContent}>
              ...
              <View style={GameStyles.modalStatusRow}>
                <StatusBadges
                  remainingHearts={
                    13 -
                    Object.values(editingRound.hearts).reduce(
                      (a, b) => a + b,
                      0,
                    )
                  }
              ...
              <ScrollView style={{ maxHeight: 400 }}>
                <View style={{ gap: 12 }}>
                  {orderedPlayersForUI.map((p) => {
                    ...
                    const isTenLocked =
                      editingRound.tenHolder !== null && !hasTen;
                    const heartsLeft =
                      13 -
                      Object.values(editingRound.hearts).reduce(
                        (a, b) => a + b,
                        ...
                        <View style={GameStyles.compactControls}>
                          ...
                          <CardToggleButton
                            ...
                            onPress={() =>
                              setEditingRound((prev) =>
                                prev
                                  ? { ...prev, tenHolder: hasTen ? null : p.id }
                                  : null,
                              )
                            }
                            style={{ marginLeft: 6 }}
                    ...
                    prev
                      ? {
                          ...
                          hearts: Object.fromEntries(
                            orderedPlayersForUI.map((p) => [p.id, 0]),
                          ),
                          qsHolder: null,
                          tenHolder: null,
                        }
...
const StatusBadges = ({
  ...
  <View style={GlobalStyles.row}>
    ...
    <View
      style={[
        GameStyles.badge,
        qsHolder ? GameStyles.badgeNeutral : GameStyles.badgeQS,
        { marginLeft: 8 },
      ]}
    >
      <Text
    ...
    <View
      style={[
        GameStyles.badge,
        tenHolder ? GameStyles.badgeNeutral : GameStyles.badgeTen,
        { marginLeft: 8 },
      ]}
    >
      <Text
...
const CardToggleButton = ({ type, active, locked, onPress, style }: any) => (
  <TouchableOpacity
    ...
    disabled={locked}
    style={[
      GameStyles.compactSpecialBtn,
      active
        ? type === "QS"
          ? GameStyles.qsActive
          : GameStyles.tenActive
        : GameStyles.specialCardInactive,
      locked && { opacity: 0.3 },
      style,
    ]}
  >
...
const LeekhaPlayerCard = ({
  ...
  <View style={GameStyles.compactCard}>
    <View style={GameStyles.compactInfo}>
      ...
      <Text style={GameStyles.roundPoints}>
    ...
    <View style={GameStyles.compactControls}>
      <HeartControl
        value={heartCount}
        onDecrement={() => onHeartChange(player.id, -1)}
        onIncrement={() => onHeartChange(player.id, 1)}
        disableDec={heartCount === 0}
        disableInc={remainingHearts === 0}
      />
      <CardToggleButton
        type="QS"
        active={hasQS}
        locked={isQSLocked}
        onPress={() => onToggleQS(player.id)}
      />
      <CardToggleButton
        type="10D"
        active={hasTen}
        locked={isTenLocked}
        onPress={() => onToggleTen(player.id)}
        style={{ marginLeft: 6 }}
      />
    </View>
  </View>
