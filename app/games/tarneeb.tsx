...
export default function TarneebScreen() {
  ...
  const commitRound = () => {
    ...
    let updatedPlayers = players.map((p, index) => {
      ...
      const isMyTeamCalling =
        (isTeamA && callingTeam === "A") || (!isTeamA && callingTeam === "B");

      // Determine partner (0<->1, 2<->3)
      const partnerIdx =
        index < 2 ? (index === 0 ? 1 : 0) : index === 2 ? 3 : 2;
    ...
    if (winningTeam) {
      ...
      updateState({
        players: updatedPlayers,
        history: newHistory,
        roundLabel: `Round ${newHistory.length + 1}`,
      });
      setCallingTeam(null);
      setBidAmount(7);
      setTricksTaken(7);
  ...
  const saveEditedRound = () => {
    ...
    const result = calculateRoundScores(
      editingRound.callingTeam,
      editingRound.bidAmount,
      editingRound.tricksTaken,
    );
    if (!result) return;
    ...
    players.forEach((p, index) => {
      ...
      const pointsChange = isTeamA ? result.pointsA : result.pointsB;
      const isMyTeamCalling =
        (isTeamA && editingRound.callingTeam === "A") ||
        (!isTeamA && editingRound.callingTeam === "B");
            ...
            {phase === "bidding" ? (
              <TouchableOpacity
                style={[
                  ...
                  !callingTeam && GameStyles.disabledButton,
                ]}
              ...
              >
                <Text
                  style={[
                    ...
                    !callingTeam && { color: Colors.textMuted },
                  ]}
                >
                  START ROUND
                </Text>
            ...
            <View style={GameStyles.modalContent}>
              ...
              <ScrollView style={{ maxHeight: 400 }}>
                <View style={{ gap: 20 }}>
                  <View>
                    <Text
                      style={[GameStyles.sectionLabel, { marginBottom: 10 }]}
                    >
                      Bidding
                    </Text>
                    <BiddingPhase
                      callingTeam={editingRound.callingTeam}
                      setCallingTeam={(t: any) =>
                        setEditingRound((prev) =>
                          prev ? { ...prev, callingTeam: t } : null,
                        )
                      }
                      bidAmount={editingRound.bidAmount}
                      setBidAmount={(b: any) =>
                        setEditingRound((prev) =>
                          prev ? { ...prev, bidAmount: b } : null,
                        )
                      }
                      setTricksTaken={() => {}}
                      teamNames={teamNames}
...
const BiddingPhase = ({
  ...
  <>
    <Text
      style={[
        GameStyles.sectionLabel,
        isModal && { fontSize: 10, marginBottom: 4 },
      ]}
    >
    ...
    <View style={[GameStyles.grid, isModal && { gap: 8 }]}>
      {["A", "B"].map((team) => (
        ...
        >
          <Text
            style={[
              GameStyles.teamNameBig,
              callingTeam === team && { color: Colors.primary },
              isModal && { fontSize: 16, marginBottom: 2 },
            ]}
          >
            Team {team}
          </Text>
          {/* Ensure teamNames is strictly accessed as string */}
          <Text style={[GameStyles.teamPlayers, isModal && { fontSize: 10 }]}>
            {teamNames[team]}
          </Text>
          {callingTeam === team && (
            <Crown
    ...
    <Text
      style={[
        GameStyles.sectionLabel,
        { marginTop: Spacing.l },
        isModal && { marginTop: Spacing.m, fontSize: 10, marginBottom: 4 },
      ]}
    >
      Call Amount
    </Text>
    <View style={[GameStyles.bidGrid, isModal && { gap: 8 }]}>
      {[7, 8, 9, 10, 11, 12, 13].map((num) => (
        ...
        >
          <Text
            style={[
              GameStyles.bidOptionText,
              bidAmount === num && { color: Colors.black },
              isModal && { fontSize: 14 },
            ]}
          >
            {num}
...
const ScoringPhase = ({
  ...
  <View
    style={[
      GameStyles.scoringContainer,
      isModal && { gap: 12, flexDirection: "row" },
    ]}
  >
    <View style={[GameStyles.counterCard, isModal && { padding: 12, flex: 1 }]}>
      ...
      <Text
        style={[
          GameStyles.counterLabel,
          isModal && { fontSize: 10, marginBottom: 8, textAlign: "center" },
        ]}
      >
        {isModal
          ? `Tricks (${callingTeam})`
          : `Tricks Taken by Team ${callingTeam}`}
      </Text>
      <View
        style={[
          GameStyles.counterRow,
          isModal && { gap: 8, justifyContent: "center" },
        ]}
      >
        <TouchableOpacity
          onPress={() => setTricksTaken((p: number) => Math.max(0, p - 1))}
          style={[
            GameStyles.counterBtn,
            isModal && { width: 32, height: 32, borderRadius: 16 },
          ]}
        >
          <Text
            style={[
              GameStyles.counterBtnText,
              isModal && { fontSize: 20, marginTop: -2 },
            ]}
          >
            -
          </Text>
        ...
        <Text
          style={[
            GameStyles.counterValue,
            isModal && { fontSize: 28, minWidth: 30 },
          ]}
        >
          {tricksTaken}
        </Text>
        <TouchableOpacity
          onPress={() => setTricksTaken((p: number) => Math.min(13, p + 1))}
          style={[
            GameStyles.counterBtn,
            isModal && { width: 32, height: 32, borderRadius: 16 },
          ]}
    ...
    {preview && (
      <View
        style={[
          GameStyles.previewCard,
          preview.success ? GameStyles.previewSuccess : GameStyles.previewFail,
          isModal && { padding: 12, flex: 1 },
        ]}
      >
        <View style={GlobalStyles.rowBetween}>
          <View style={GlobalStyles.row}>
            {preview.success ? (
              <Check size={isModal ? 16 : 24} color={Colors.primary} />
            ) : (
              <X size={isModal ? 16 : 24} color={Colors.danger} />
            )}
            <Text
              style={[
                GameStyles.previewTitle,
                isModal && { fontSize: 14, marginLeft: 6 },
              ]}
            >
              {preview.success ? "Success" : "Failed"}
            </Text>
          </View>
        </View>
        <View style={[GameStyles.divider, isModal && { marginVertical: 8 }]} />
        <View style={GlobalStyles.rowBetween}>
          <Text style={[GameStyles.previewSub, isModal && { fontSize: 10 }]}>
            Team A
          </Text>
          <Text
            style={[
              GameStyles.previewSubScore,
              { color: preview.pointsA >= 0 ? Colors.text : Colors.danger },
              isModal && { fontSize: 14 },
            ]}
          >
            {preview.pointsA > 0 ? "+" : ""}
        ...
        <View style={[GlobalStyles.rowBetween, { marginTop: isModal ? 4 : 8 }]}>
          ...
          <Text
            style={[
              GameStyles.previewSubScore,
              { color: preview.pointsB >= 0 ? Colors.text : Colors.danger },
              isModal && { fontSize: 14 },
            ]}
          >
            {preview.pointsB > 0 ? "+" : ""}
