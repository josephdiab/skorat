...
export default function FourHundredScreen() {
  ...
  const commitRound = () => {
    ...
    const { finalHistory, updatedPlayers: replayPlayers } = replay400History(players, preliminaryHistory);
    ...
    const teamA_Qualified =
      (p0.totalScore >= limit && p1.totalScore > 0) || (p1.totalScore >= limit && p0.totalScore > 0);
    const teamB_Qualified =
      (p2.totalScore >= limit && p3.totalScore > 0) || (p3.totalScore >= limit && p2.totalScore > 0);
    ...
    if (winningTeam) {
      ...
      updateState({ players: updatedPlayers, history: finalHistory, status: "completed" });
    } else {
    ...
    if (__DEV__) {
      ...
      const testStats = StatsEngine.get400Stats([{ ...gameState!, history: finalHistory }], myProfileId);
      Logger.info("SANITY_STATS", "400 Stats", { profileId: myProfileId, ...testStats });
    }
  };
  ...
  const saveEditedRound = () => {
    ...
    const updatedDetails: Record<string, GameRoundDetails> = { ...oldRound.playerDetails };
    ...
    const { finalHistory, updatedPlayers: replayPlayers } = replay400History(players, newHistory);
    let updatedPlayers = replayPlayers;
    ...
    if (winningTeam) {
      ...
      updateState({ players: updatedPlayers, history: finalHistory, status: "active" });
    }
    ...
    if (__DEV__) {
      const myProfileId = players[0].profileId;
      const testStats = StatsEngine.get400Stats([{ ...gameState!, history: finalHistory }], myProfileId);
      Logger.info("SANITY_STATS", "400 Stats (after edit)", { profileId: myProfileId, ...testStats });
    }
  };
                      ...
                      <View key={p.id} style={GameStyles.editRow}>
                        ...
                        <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                          <TouchableOpacity
                            onPress={() =>
                              setEditingRound((prev) =>
                                prev ? { ...prev, bids: { ...prev.bids, [p.id]: Math.max(2, bid - 1) } } : null
                              )
                            }
                          >
                          ...
                          <TouchableOpacity
                            onPress={() =>
                              setEditingRound((prev) =>
                                prev ? { ...prev, bids: { ...prev.bids, [p.id]: Math.min(13, bid + 1) } } : null
                              )
                            }
                          >
                            <Text style={GameStyles.editBtn}>+</Text>
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() =>
                            setEditingRound((prev) =>
                              prev ? { ...prev, results: { ...prev.results, [p.id]: !passed } } : null
                            )
                          }
                        >
...
const BiddingCard = ({ player, bid, onAdjust }: { player: Player; bid: number; onAdjust: (d: number) => void }) => (
  <View style={GameStyles.biddingCard}>
    ...
    <View style={GameStyles.controlsRowCompact}>
      <TouchableOpacity onPress={() => onAdjust(-1)} style={GameStyles.controlBtnSmall}>
